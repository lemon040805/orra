...
def handler(event, context):
    try:
        ...
        if 'multipart/form-data' in content_type:
            # Handle audio analysis for pronunciation feedback
            body = event.get('body', '')
            ...
            # Start transcription for pronunciation analysis
            ...
            while wait_time < max_wait:
                ...
                if status == 'COMPLETED':
                    ...
                    # Generate feedback
                    feedback = generate_pronunciation_feedback(transcription, confidence)
                    ...
                    return {
                        ...
                        'body': json.dumps({
                            ...
                            'feedback': feedback,
                            'suggestions': get_pronunciation_suggestions(transcription, confidence),
                            'timestamp': datetime.utcnow().isoformat(),
                            'method': 'aws_transcribe_analysis'
                        })
                    }
            ...
            return get_fallback_pronunciation_response()
...
def generate_pronunciation_feedback(transcription, confidence):
    if confidence > 0.9:
...
def get_pronunciation_suggestions(transcription, confidence):
    if confidence < 0.7:
...
def get_fallback_pronunciation_response():
    return {
        'statusCode': 200,
        'headers': {
            'Content-Type': 'application/json',
        ...
        'body': json.dumps({
            ...
            'confidence': 0.85,
            'feedback': 'Good pronunciation practice! Keep working on your accent.',
            'suggestions': 'Try to roll your R sounds more clearly.',
            'timestamp': datetime.utcnow().isoformat(),
            'method': 'fallback'
        })
    }
